...
#
#light changes in 2017 to deal with upgrades to Zotero 5 (mostly to the database schema) 
#and changes introduced by Alfred 3; all old code retained in comments and new code marked by 'mnf'
...
class ZotCache(object):
    ...
    def setup(self):
        ...
        if os.path.exists(self.wf_obj.datafile("first-run.txt")):
            ...
            scpt_str = 'tell application "Alfred 3" to search "z:config"'
            run_applescript(scpt_str)
            return False
    ...
    def creator_query(self, creator_data_id, creator_type_id):
        ...
        creator_info_sql = """SELECT creators.lastName, 
            creators.firstName, creatorTypes.creatorType 
            FROM creators, creatorTypes
            WHERE
                creatorID = {0}
    ...
    def get_cache(self):
        ...
        for basic in basic_info:
            ...
            for _creator in creators_data:
                ...
                #MNF creators_id_query = ['creatorDataID', 'creators',
                 #                   'creatorID', creator_id]
                creators_id_query = ['creatorID', 'creators', 
                					'creatorID', creator_id]
            ...
            for _tag in tags_data:
                tag_id = ''
                (tag_id,) = _tag
                tag_info_query = ['name, tagID', 'tags', 'tagID', tag_id]
                (tag_name,
            ...
            ##### Notes Info
            item_dict['notes'] = []
            note_info_query = ['note', 'itemNotes', 'parentItemID', item_id]
            ####################mnf sourceitemID
